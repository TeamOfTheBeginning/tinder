...
const Post = (props) => {
    ...
    useEffect(
        ()=>{
            ...
            setPostWriter( getNickname( props.post.writer ) )
    ...
    return (
        <div className='post'>
            <div className='writer' style={{display:"flex"}}>
                ...
                {
                    ( 
                        ( props.post.writer != loginUser.id) &&
                ...
                <Slider {...settings}>
                    {
                        (imgList)?(
                            imgList.map((img, idx)=>{
                                return (
                                    <img key={idx} src={`http://localhost:8070/images/${img.savefilename}`} width="750" height="700"/>
                                )
                            })
                        ):(null)
                    }    
